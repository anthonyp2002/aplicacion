...
class ProlecRAController extends GetxController {
  ...
  String? enunciado =
      'Presiona la palabra con significado contrario de la palabra mostrada.';
  String? engine;
  double volume = 0.5;
  double pitch = 1.0;
  double rate = 0.5;
  ...
  void nextQuestions() {
    if (pageController.positions.isNotEmpty) {
      if (pageController.page == AntoModel().seuModel.length - 1) {
        flutterTts.stop();
        Get.offAllNamed('/prolecRB');
        Get.find<ProlecRBController>()
            .datos(use, tiempo, puntos, puntosH, puntosO, puntosIA, puntuacion);
      } else {
        _isPressed = false;
